...
public class Equalizer implements PcmFilterFactory {
    ...
    public Equalizer() {
        this.bandMultipliers = new float[BAND_COUNT];
    }
    ...
    public List<AudioFilter> buildChain(AudioTrack track, AudioDataFormat format, UniversalPcmAudioFilter output) {
        if (isCompatible(format)) {
            return Collections.singletonList(new EqualizerFilter(format.channelCount, output, bandMultipliers));
        } else {
            return Collections.emptyList();
        }
    }

    private static boolean isCompatible(AudioDataFormat format) {
    ...
    public class EqualizerFilter implements FloatPcmAudioFilter {
    ...
    private static class ChannelProcessor {
        ...
        private ChannelProcessor(float[] bandMultipliers) {
            this.history = new float[Equalizer.BAND_COUNT * 6];
        ...
        private void process(float[] samples, int startIndex, int endIndex) {
            for (int sampleIndex = startIndex; sampleIndex < endIndex; sampleIndex++) {
                ...
                for (int bandIndex = 0; bandIndex < Equalizer.BAND_COUNT; bandIndex++) {
                    int x = bandIndex * 6;
                    int y = x + 3;
                ...
                samples[sampleIndex] = result * 4.0f;
